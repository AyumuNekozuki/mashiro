...
const dcClient = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.MessageContent, GatewayIntentBits.GuildMessages] });
...
const Mashiro_discord_Core = (token) => {
	...
	dcClient.once('ready', async () => {
		...
		dcClient.user.setActivity('Misskeyを眺めているよ', { type: ActivityType.Watching });
